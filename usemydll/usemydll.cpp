...
int main()
{
	...
	*/
	...
	HANDLE threaHandle = CreateRemoteThread(processHandle, securityDescriptor, stackSize, MyThreadFunction, arg, flags, threadID);
...
DWORD WINAPI MyThreadFunction(LPVOID lpParam)
{
	LoadLibraryA(static_cast<LPCSTR>(lpParam));
	return 0;
}
